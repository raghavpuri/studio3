...
const updatePage = async () => {
  ...
  console.log(fruitsArray[1].nutritions.sugar)
  const dispArr = fruitsArray.filter(customFunc)

  function customFunc(obj){
    return obj.nutritions.sugar >= 15
  }

  const nameArr = dispArr.map(customFunctwo)

  function customFunctwo(obj){
    return obj.name
  }
  ...
  const exEl = document.getElementById('text-disp')
  exEl.append(newEl);
